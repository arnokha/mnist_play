##
## Benchmark the "mold matching" method of digit classification (pixel intensities of molds are regularized)
## We want to measure:
...
from create_digit_molds import create_regularized_molds
...
r_molds = create_regularized_molds()
...
##
print("Testing regularized molds on training data...")
score = 0
...
for i in range(mnist.n_train):
	for j in range(mnist.n_labels):
		label_score[j] = np.sum( np.multiply(mnist.x_train[i,:], r_molds[j,:]) ) 
...
##
## Test regularized molds on test data
##
print("Testing regularized molds on test data...")
score = 0

start = time.perf_counter()
for i in range(mnist.n_test):
	for j in range(mnist.n_labels):
		label_score[j] = np.sum( np.multiply(mnist.x_test[i,:], r_molds[j,:]) ) 
	prediction = np.argmax(label_score)
...
## Report Metrics
print()
print("Total training time (time to create regularized molds) was: " + str(total_training_time) + " seconds")
